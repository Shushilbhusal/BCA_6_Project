...
export const getDataHandler = async (req: Request, res: Response) => {
    try {
        ...
        const topSellingProducts = highestSales.length > 0 ? highestSales : [{ message: "No sales data available" }];
        ...
        res.status(200).json({ message: "Data fetched successfully", dashboardData });
    } catch (error) {
